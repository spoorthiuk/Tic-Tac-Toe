# As I was practicing OOP, I thought of rewriting previous code using these concepts
...
# The following class is used for formatting the tes
class TextFormat:
    def __init__(self):
        self.red = "\033[1;31m"
        self.reset = "\033[0m"
...
# The following class holds all the game related variables and functions
class TicTacToe:
    def __init__(self):
        self.m = 3
        self.n = 3
    ...
    def print_win_case(self, A, m1, n1, m2, n2, m3, n3):
        print("\n")
        for i in range(0, 3):
            for j in range(0, 3):
                if ((i == m1) and (j == n1)) or ((i == m2) and (j == n2)) or ((i == m3) and (j == n3)):
                    if A[i][j] == 1:
                        print(self.red + " X " + self.reset, end='')
                    elif A[i][j] == 2:
                        print(self.red + " O " + self.reset, end='')
                else:
                    if A[i][j] == 1:
                        print(" X ", end='')
                    elif A[i][j] == 2:
    ...
    def check_results(self, A):
        for i in range(0, 3):
            if A[i][0] == A[i][1] and A[i][1] == A[i][2] and A[i][0] != 0:
                self.print_win_case(A, i, 0, i, 1, i, 2)
                return A[i][0]
            if A[0][i] == A[1][i] and A[1][i] == A[2][i] and A[i][0] != 0:
                self.print_win_case(A, 0, i, 1, i, 2, i)
                return A[0][i]
        if A[0][0] == A[1][1] and A[1][1] == A[2][2] and A[0][0] != 0:
            self.print_win_case(A, 0, 0, 1, 1, 2, 2)
            return A[0][0]
        if A[2][0] == A[1][1] and A[1][1] == A[0][2] and A[2][0] != 0:
            self.print_win_case(A, 2, 0, 1, 1, 0, 2)
            return A[2][0]
        return 0
